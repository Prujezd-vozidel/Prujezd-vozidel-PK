angular.module('pvpk')
  .controller('infoController', ['$rootScope', '$scope', '$location', 'config', 'Device', 'Vehicle', 'Range', function ($rootScope, $scope, $location, config, Device, Vehicle, Range) {
    this.$onInit = function () {
      ...
      Vehicle.query(null).$promise.then(function (dataResponse) {
        $scope.vehicles = dataResponse
      }, function (response) {
    ...
    $rootScope.$on('setRangeFromUrl', function (event, args) {
      ...
      $scope.range = {
        ...
        isTime: params.isTime != 0,
        maxDate: $scope.range == null ? null : $scope.range.maxDate,
        minDate: $scope.range == null ? null : $scope.range.minDate
      }

      if (!$scope.isLoadRange) {
        Range.get().$promise.then(function (dataResponse) {
          $scope.range.fromDate = moment.max(moment(dataResponse.last_date).add(config.DEFAULT_RANGE_DATE_DAY.from, 'd'), moment(dataResponse.first_date)).toDate()
          $scope.range.toDate = moment.min(moment($scope.range.toDate), moment(dataResponse.last_date)).toDate()
          $scope.range.maxDate = moment(dataResponse.last_date).toDate()
          $scope.range.minDate = moment(dataResponse.first_date).toDate()
    ...
    $rootScope.$on('infoLocation', function (event, args) {
      ...
      Device.get(query).$promise.then(function (dataResponse) {
        $rootScope.selectDevice = dataResponse
        $scope.renderGraph()
